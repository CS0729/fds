...
int MinFromSettoVertex(int Set[],int num,int m){
    int i;
    int mindis;
    for(i=0;i<num;i++){                 //we first initialize the minimum distance,noting that the distance!=0 
        if(g[Set[i]][m]!=0){            //because 0 means there's no edge between 2 vertices
            mindis=g[Set[i]][m];
            break;
        }   
    }
...
int MinFromSettoRemainset(int Set[],int Remainset[],int num,int v){
   int i,j;
   int mindis;
    for(i=0;i<num;i++){                        //we also first initialize the minimum distance,noting that the distance!=0 
       for(j=0;j<(v-num);j++){                 //because 0 means there's no edge between 2 vertices
